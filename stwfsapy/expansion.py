...
_any_case_from_braces_expression = re.compile(r"\\\((.{3,}?)\\\)")
...
def _expand_ampersand_with_spaces_fun(label: str) -> str:
    return _ampersand_abbreviation_matcher.sub(r"\g<1> ?& ?\g<2>", label)
...
def collect_expansion_functions(
    ...
    options = (
        (
            ...
            _replace_by_pattern_fun(_upper_case_abbreviation_from_braces_expression),
        ),
        (
            extract_any_case_from_braces,
            _replace_by_pattern_fun(_any_case_from_braces_expression),
        ),
        (expand_ampersand_with_spaces, _expand_ampersand_with_spaces_fun),
        (
