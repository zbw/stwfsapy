...
def make_test_result_matrix(values):
    return csr_matrix(
        (
            values,
            (
                [i for i, count in enumerate(_doc_counts) for _ in range(count)],
...
def no_match_predictor(mocker):
    predictor = p.StwfsapyPredictor(None, None, None, None)
    predictor.concept_map_ = _concept_map
    predictor.match_and_extend = mocker.Mock(return_value=([], [0, 0, 0]))
    return predictor
...
def case_graph():
    g = Graph()
    g.add((c.test_concept_ref_0_0, SKOS.prefLabel, Literal("Three word label")))
    return g


def test_result_collection():
    res = p.StwfsapyPredictor._collect_prediction_results(
        _predictions[:, 1], [c[0] for c in _concepts_with_text], _doc_counts
    )
...
def test_sparse_matrix_creation():
    ...
    for i, count in enumerate(_doc_counts):
        ...
        assert list(row.nonzero()[1]) == list(
            reversed([22 - i for i in _concepts[slice_start : slice_start + count]])
        )
...
def test_match_and_extend_with_truth_with_vec(patched_dfa, mock_vectorizer):
    ...
    txt_indices = [idx for idx, txt in enumerate(in_txts) for _ in range(len(txt))]
    for txt_idx, match, expected in zip(txt_indices, matches, expected_matches):
        check_fit_arg(
...
def test_init_and_fit_with_vec(full_graph, mocker):
    ...
    assert isinstance(
        predictor.pipeline_.named_steps["Classifier"], DecisionTreeClassifier
    )
    ...
    for txt_idx, actual, expected in zip(
        txt_indices, pipe_fit_args[0], expected_features
    ):
        check_fit_arg(
            predictor.text_vectorizer_,
            predictor.text_features_,
...
def test_match_and_extend_with_truth_without_vec(patched_dfa):
    ...
    txt_indices = [idx for idx, txt in enumerate(in_txts) for _ in range(len(txt))]
    for txt_idx, match, expected in zip(txt_indices, matches, expected_matches):
        check_fit_arg(None, predictor.text_features_, in_txts[txt_idx], match, expected)
    assert ys == [0, 0, 1, 0, 1, 0]
...
def test_init_and_fit_without_vec(full_graph, mocker):
    ...
    for txt_idx, actual, expected in zip(
        ...
        check_fit_arg(
            None, predictor.text_features_, train_texts[txt_idx], actual, expected
        )
...
def check_fit_arg(vec_fun, text_feature_fun, txt, actual, expected):
    ...
    if vec_fun:
        assert (actual[2].toarray() == vec_fun.transform([txt]).toarray()).all()
    else:
        assert actual[2] == 0
...
def test_predict(mocked_predictor):
    ...
    assert (res.toarray() == make_test_result_matrix(_classifications).toarray()).all()
    mocked_predictor.match_and_extend.assert_called_once_with([])
    mocked_predictor.pipeline_.predict.assert_called_once_with(_concepts_with_text)


def test_predict_proba(mocked_predictor):
    res = mocked_predictor.predict_proba([])
    assert (
        res.toarray() == make_test_result_matrix(_predictions[:, 1]).toarray()
    ).all()
    mocked_predictor.match_and_extend.assert_called_once_with([])
...
def test_expansion(full_graph, mocker):
    ...
    mocker.patch("stwfsapy.expansion.collect_expansion_functions", mock_expander)
    predictor = p.StwfsapyPredictor(
        full_graph,
        c.test_type_concept,
        c.test_type_thesaurus,
...
def test_mark_doc_end():
    ...
    assert matches == [("a", None, None, [0], 5, 0), ("b", None, None, [2, 5], 7, 1)]
...
def test_serialization_inversion_without_vec(tmpdir, full_graph):
    ...
    assert loaded.simple_english_plural_rules == (predictor.simple_english_plural_rules)
    ...
    assert loaded.thesaurus_relation_type_uri == (predictor.thesaurus_relation_type_uri)
    ...
    loaded_txt_feat_names = [name for name, _ in loaded.text_features_.transformer_list]
    pred_txt_feat_names = [
...
def test_warning(mocker):
    ...
    p._handle_construction(con_state, "test_concept", "invalid_label")
