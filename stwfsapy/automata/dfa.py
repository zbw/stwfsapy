...
class State:
    ...
    def __init__(
        self, symbol_transitions=None, non_word_char_transition=None, accepts=None
    ):
    ...
    def to_dict(self, acceptance_handler: Callable) -> Dict[str, Any]:
        return {
    ...
    def __eq__(self, other):
        return (
            ...
            and (self.symbol_transitions == other.symbol_transitions)
            and (self.non_word_char_transition == other.non_word_char_transition)
        )

    @staticmethod
    def from_dict(conf: Dict[str, Any], acceptance_handler: Callable):
        return State(
            symbol_transitions=conf[_KEY_STATE_SYMBOL_TRANSITIONS],
            non_word_char_transition=conf.get(_KEY_STATE_NON_WORD_CHAR_TRANSITION),
            accepts=[acceptance_handler(accept) for accept in conf[_KEY_STATE_ACCEPTS]],
        )
...
class Dfa:
        ...
        for start in range(len(search_text)):
            ...
            while len(stack) > 0:
                ...
                for accept in state.accepts:
                    ...
                    yield (accept, text[start:original_end], start, original_end)
                ...
                if not symbol.isalnum() and state.non_word_char_transition:
                    stack.append((state.non_word_char_transition, position + 1))
                try:
    ...
    def to_dict(self, acceptance_handler: Callable) -> Dict[str, Any]:
        return {
            _KEY_DFA_STATES: [
                state.to_dict(acceptance_handler) for state in self.states
            ]
        }
    ...
    def from_dict(conf: Dict[str, Any], acceptance_handler: Callable):
        return Dfa(
            [
                ...
                for state_conf in conf[_KEY_DFA_STATES]
            ]
        )
